...
| 3 Payloads/SDK | ✅ Mostly | Registry validated server-side (`x-` passthrough). `@buddylist/sdk` (Node+browser) with `on(payload_type)`, `reply()`, `request()` (correlated by task_id/question_id **or** `reply_to`). Directory search. **Not yet:** Python SDK, `buddylist-mcp`. |
...
Verified end-to-end: two SDK agents + a human in the retro client; agent→agent `request()`/`review.result`; human `/task` → `task.accept` → `task.result` rendered as cards. 12 integration tests green (`npm test`).
